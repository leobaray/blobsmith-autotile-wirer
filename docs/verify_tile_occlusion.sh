#!/usr/bin/env bash
# Runs every claim in docs/why-my-tiles-cast-no-shadow.md against a real Godot 4
# binary. Exits non-zero if any claim fails.
#
#   docs/verify_tile_occlusion.sh /path/to/Godot_v4.7-stable_linux.x86_64
#
# Cannot use --headless: whether a pixel is in shadow is read back from a
# rendered SubViewport, and the headless dummy renderer returns no image. It asks
# for a real GL context on a virtual X server.
#
...
config/name="BlobsmithTileOcclusionCheck"
EOF
cp "$HERE/verify_tile_occlusion.gd" "$PROJ/verify_tile_occlusion.gd"

OUT="$(timeout 180 xvfb-run -a -s "-screen 0 640x480x24" \
  "$GODOT" --rendering-driver opengl3 --path "$PROJ" --script verify_tile_occlusion.gd 2>&1)"
echo "$OUT" | grep -E '^(Godot Engine v|PASS |FAIL |INFO |TILE OCCLUSION)'

if ! printf '%s\n' "$OUT" | grep -q '^TILE OCCLUSION: '; then
  echo "ERROR: the script did not finish — no claim was measured. Full output:" >&2
  printf '%s\n' "$OUT" >&2
  exit 3
fi

printf '%s\n' "$OUT" | grep -q '^TILE OCCLUSION: ALL PASS' || exit 1
exit 0
