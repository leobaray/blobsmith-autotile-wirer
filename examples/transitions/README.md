# Free terrain-transition TileSets — grass/sand, sand/water shoreline, stone/grass path, water/grass pond, grass/stone overgrown floor, dirt/grass road, grass/dirt tilled ground, snow/grass winter edge, lava/stone volcano floor — Godot 4
...
seam between them. Nine pairs, each in 16px and 32px.
**MIT: use it in commercial games, no credit required.**

**[⬇ All eighteen in one zip (516 KB)](https://github.com/leobaray/blobsmith-autotile-wirer/releases/download/transitions-pack-v2/godot-47blob-transitions-pack.zip)**
...
| `snow_on_grass_47blob_16px.png` + `.tres` | snow over grass (winter maps, snowfields, mountain tops) | 16px | 128×96 |
| `snow_on_grass_47blob_32px.png` + `.tres` | snow over grass (winter maps, snowfields, mountain tops) | 32px | 256×192 |
| `lava_on_stone_47blob_16px.png` + `.tres` | lava over stone (volcanoes, dungeon hazards) | 16px | 128×96 |
| `lava_on_stone_47blob_32px.png` + `.tres` | lava over stone (volcanoes, dungeon hazards) | 32px | 256×192 |
...
![snow over grass](snow_on_grass_47blob_32px.png) ![lava over stone](lava_on_stone_47blob_32px.png)
...
for the road Dirt and Grass, for the tilled ground Grass and Dirt, for the
winter edge Snow and Grass, for the volcano floor Lava and Stone.
...
eighteen PNGs and, for each TileSet, paints a 14×11 field of the under terrain with a
...
2026-09-14 (16/16); seven pairs on 2026-09-15 (112/112); all nine pairs on
2026-09-17, **144/144 checks on each of Godot 4.3, 4.4 and 4.7 stable**:
...
T6 was also run inverted (`LG_SELFTEST=1`): exit 1 with exactly the eighteen T6
...
grass block with its four colours swapped, snow the sand block, lava the water
block).
