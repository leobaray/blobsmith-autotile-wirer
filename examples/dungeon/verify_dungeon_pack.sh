#!/usr/bin/env bash
# Runs every claim in examples/dungeon/README.md against a real Godot 4 binary,
# with the real TileSets, in a throwaway project.
#
#   examples/dungeon/verify_dungeon_pack.sh /path/to/Godot_v4.x-stable_linux.x86_64
#   examples/dungeon/verify_dungeon_pack.sh /path/to/godot --invert   # must exit 1
#
...
config/name="BlobsmithDungeonPackCheck"
EOF
mkdir "$PROJ/dungeon"
cp "$HERE"/*.png "$HERE"/*.tres "$HERE/manifest.json" "$PROJ/dungeon/"
cp "$HERE/verify_dungeon_pack.gd" "$PROJ/verify_dungeon_pack.gd"
...
OUT="$(timeout 300 "$GODOT" --headless --path "$PROJ" --script verify_dungeon_pack.gd -- "$@" 2>&1)"
printf '%s\n' "$OUT" | grep -E '^(Godot |PASS |FAIL |MANIFEST |ROOM |DUNGEON)'

if ! printf '%s\n' "$OUT" | grep -qE '^DUNGEON'; then
...
E="$(printf '%s\n' "$OUT" | grep -cE 'ERROR')"
if [ "$E" = "0" ]; then
  echo "PASS P0  the verification run (4 TileSets, load, paint, faces from pixels) prints 0 ERROR / SCRIPT ERROR lines"
else
...
printf '%s\n' "$OUT" | grep -qE '^DUNGEON [0-9]+/[0-9]+ PASS' || exit 1
exit 0
