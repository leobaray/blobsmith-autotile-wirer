...
#
#   examples/isometric/verify_hex_pack.sh /path/to/Godot_v4.x-stable_linux.x86_64
#   examples/isometric/verify_hex_pack.sh /path/to/godot --invert   # must exit 1
#
...
config/name="BlobsmithHexPackCheck"
EOF
mkdir "$PROJ/hex"
cp "$HERE"/*.png "$HERE"/*.tres "$HERE/manifest.json" "$PROJ/hex/"
cp "$HERE/verify_hex_pack.gd" "$PROJ/verify_hex_pack.gd"
...
OUT="$(timeout 300 "$GODOT" --headless --path "$PROJ" --script verify_hex_pack.gd -- "$@" 2>&1)"
printf '%s\n' "$OUT" | grep -E '^(Godot |PASS |FAIL |MANIFEST |HEX)'

if ! printf '%s\n' "$OUT" | grep -qE '^HEX'; then
...
E="$(printf '%s\n' "$OUT" | grep -cE 'ERROR')"
if [ "$E" = "0" ]; then
  echo "PASS P0  the verification run (16 TileSets, load, paint, neighbour agreement) prints 0 ERROR / SCRIPT ERROR lines"
else
...
printf '%s\n' "$OUT" | grep -qE '^HEX [0-9]+/[0-9]+ PASS' || exit 1
exit 0
