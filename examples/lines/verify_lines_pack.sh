#!/usr/bin/env bash
# Runs every claim in examples/lines/README.md against a real Godot 4 binary,
# with the real TileSets, in a throwaway project.
#
#   examples/lines/verify_lines_pack.sh /path/to/Godot_v4.x-stable_linux.x86_64
#   examples/lines/verify_lines_pack.sh /path/to/godot --invert   # must exit 1
#
...
EOF
mkdir "$PROJ/lines"
cp "$HERE"/*.png "$HERE"/*.tres "$HERE/manifest.json" "$PROJ/lines/"
cp "$HERE/verify_lines_pack.gd" "$PROJ/verify_lines_pack.gd"
...
OUT="$(timeout 300 "$GODOT" --headless --path "$PROJ" --script verify_lines_pack.gd -- "$@" 2>&1)"
printf '%s\n' "$OUT" | grep -E '^(Godot |PASS |FAIL |MANIFEST |LINES)'

if ! printf '%s\n' "$OUT" | grep -qE '^LINES'; then
...
E="$(printf '%s\n' "$OUT" | grep -cE 'ERROR')"
if [ "$E" = "0" ]; then
  echo "PASS P0  the verification run (8 TileSets, load, connect, path) prints 0 ERROR / SCRIPT ERROR lines"
else
...
printf '%s\n' "$OUT" | grep -qE '^LINES [0-9]+/[0-9]+ PASS' || exit 1
exit 0
